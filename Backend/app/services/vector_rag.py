...
def _get_collection():
    ...
    try:
        ...
        expected_count = len(LEGAL_KNOWLEDGE_BASE)
        stored_count = _collection.count()
        if stored_count != expected_count:
            # Stale collection: the KB content changed size since this
            # collection was built (docs added/removed), which also means
            # the TF-IDF vectorizer's fitted vocabulary has likely changed.
            # Reusing embeddings from the old vocabulary would silently
            # produce meaningless similarity scores instead of a clean
            # error, so rebuild from scratch rather than trust stale data.
            _client.delete_collection("cyber_legal_kb_tfidf")
